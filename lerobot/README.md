...
## 🔄 Syncing Upstream LeRobot While Preserving Custom Modifications

This fork contains custom modifications. Here's how to safely integrate upstream LeRobot updates without losing your work.
...
git diff --name-status HEAD...upstream/main > changes.txt
...
# 5. Merge (review before committing)
...
|---------------|--------|
| Your new files (custom envs/scripts) | `git checkout --ours <file>` |
| Upstream-only files | `git checkout --theirs <file>` |
| Modified by both | Manual merge (keep both changes) |
...
# - Keep your custom code + integrate upstream improvements
...
Create `.gitattributes` to auto-protect fully custom files:

```bash
cat >> .gitattributes << 'EOF'
# Custom files - always keep our version during merges
lerobot/envs/your_custom_env.py merge=ours
lerobot/datasets/your_custom_script.py merge=ours
EOF
...
grep -r "<<<<<<< HEAD" lerobot/ || echo "✓ Clean"

# Test your custom functionality
python -c "import lerobot; print('✓ Import works')"
python your_custom_script.py --help

# Commit if tests pass
git commit -m "Merge upstream LeRobot $(date +%Y-%m-%d)"
git tag merge-upstream-$(date +%Y%m%d)
...
git log upstream/main --oneline | grep "feature name"

# Apply it
git cherry-pick <commit-hash>

# Or apply partial file changes
git checkout -p upstream/main -- path/to/file.py
```
...
```bash
git merge --abort
# Then cherry-pick changes incrementally
```

**Lost custom changes?**
```bash
git diff backup-20250930 HEAD -- your_file.py
git checkout backup-20250930 -- your_file.py
```
...
git bisect start
git bisect bad HEAD
git bisect good backup-20250930
```
