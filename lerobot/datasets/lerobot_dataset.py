    ...
    def _get_query_timestamps(
        ...
        for key in self.meta.video_keys:
            if query_indices is not None and key in query_indices:
                # The transform converts each item to a tensor, so we stack the list of tensors.
                timestamps_tensors = [self.hf_dataset[i]["timestamp"] for i in query_indices[key]]
    ...
    def _query_hf_dataset(self, query_indices: dict[str, list[int]]) -> dict:
        ...
        for key, q_idx in query_indices.items():
            if key in self.meta.video_keys:
                continue
            # The hf_dataset has a transform that converts samples to tensors.
            # We retrieve samples one by one and stack them into a single tensor.
            item[key] = torch.stack([self.hf_dataset[i][key] for i in q_idx])
        return item
