...
To train a policy to control your robot, use the [`lerobot-train`](../src/lerobot/scripts/lerobot_train.py) script. A few arguments are required. Here is an example command:
...
You can use the `record` function from [`lerobot/scripts/lerobot_teleoperate.py`](../src/lerobot/scripts/lerobot_teleoperate.py) but with a policy checkpoint as input. For instance, run this command to record 10 evaluation episodes:
