...
def rollout(
    ...
    # Try to get max_episode_steps, fallback for MetaWorld environments
    try:
        max_steps = env.call("_max_episode_steps")[0]
    except AttributeError:
        # MetaWorld environments don't have _max_episode_steps, use default of 500
        max_steps = 500
    progbar = trange(
        max_steps,
    ...
    while not np.all(done) and step < max_steps:
        ...
        # Extract success from info. MetaWorld puts `_success` directly in info, not in final_info.
        # Check the root level of info first (for MetaWorld), then fall back to final_info for other envs.
        successes = [False] * env.num_envs
        
        # Check root-level success fields (MetaWorld uses this pattern)
        if "_success" in info:
            successes_array = np.asarray(info["_success"])
            if successes_array.shape == (env.num_envs,):
                successes = (successes_array > 0).tolist()
        elif "is_success" in info:
            successes_array = np.asarray(info["is_success"])
            if successes_array.shape == (env.num_envs,):
                successes = (successes_array > 0).tolist()
        elif "success" in info:
            successes_array = np.asarray(info["success"])
            if successes_array.shape == (env.num_envs,):
                successes = (successes_array > 0).tolist()
        
        # Also check final_info for other environment types
        if "final_info" in info:
            for idx, final_info in enumerate(info["final_info"]):
                if final_info is not None:
                    # Check multiple success field names (OR logic for robustness)
                    success = (
                        final_info.get("is_success", False)
                        or final_info.get("success", False)
                        or final_info.get("_success", False)
                    )
                    successes[idx] = successes[idx] or bool(success)  # OR with root-level
